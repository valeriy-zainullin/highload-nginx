...
Для начала будем смотреть только на api текущей даты `what-date-is-it`. Мы видим, что лучшие результаты, за исключением случая `nginx-setup-2-withcache`, дает просто обращение к бекэнду напрямую, без балансировок. Сделаем об этом выводы ниже.

Кеширование помогает, потому что до бекэнда второй и последующие запросы не доходят, nginx, конечно же, быстрее ответит из кеша, чем go создаст ответ.

Для api `what-is-my-name` кеширование не настроено, т.к. он принимает `POST` запросы. А с кешированием `POST` запросов есть сложности: нельзя выдавать один и тот же ответ для всех клиентов на прокси, т.к. могут утекать личные данные (т.е. кеширование для каждого пользователя в отдельности -- уже не очень хорошо, мб), так еще и поддерживается не всеми клиентами ([нашел такую информацию](https://stackoverflow.com/questions/626057/is-it-possible-to-cache-post-methods-in-http)).
...
Видно, что польза есть. Ускорение в полтора раза. Хотя эта величина не будет зависеть от бекенда (только для первого запроса время зависит от бекенда), т.к. при выдаче кешированного ответа к бекенду даже не будет запроса.


### Второй бенчмарк
Теперь бекенд на питоне. Доступные ресурсы -- шесть ядер. Каждый instance бекэнда может задействовать два. В реальной жизни -- это разные машины по два (в скобочках, 20) ядра. Теперь балансировка только позволит больше ресурсов включить в работу (главное, чтобы накладные расходы на балансировку, пересылки запросов и прочее, не мешали получить выгоду).
...
Будем для начала смотреть только на `what-is-my-name`. Балансировка действительно помогает, в два раза увеличивает количество запросов в секунду -- ускоряет каждый запрос в среднем в два раза. Но не в три, хотя instance-ов бекенда три.

Кеширование помогает, цифры похожи на первый бенчмарк. Объяснимо, ведь запрос до бекенда по сути не доходит, а мы используем тот же самый nginx между бенчмарками, с одинаковыми настройками кеширования.

##### Балансировка

Ускорение в два раза! Т.е. буквально балансировка включает в работу дополнительные ресурсы. Так и бывает в реальной жизни.
...
Помогает, цифры похожи на первый бенчмарк.
